package algorithm.array;

/************************************************************************************
 * 功能描述：
 *
 * 输入一个整形数组，数组里有正数也有负数。
 * 数组中连续的一个或多个整数组成一个子数组，每个子数组都有一个和。
 * 求所有子数组的和的最大值，要求时间复杂度为O(n)。
 * 例如输入的数组为1, -2, 3, 10, -4, 7, 2, -5，和最大的子数组为3, 10, -4, 7, 2，
 * 因此输出为该子数组的和18。
 *
 * 创建人：岳增存  yuezc@seentao.com
 * 创建时间： 2017年08月15日 --  下午8:27 
 * 其他说明：
 * 修改时间：
 * 修改人：
 *************************************************************************************/
public class MaxChildArraySum {


    /**
     * 暴力循环
     * @param array
     * @return
     */
    public int maxChildArraySum1(int[] array){
        int max = array[0];

        int currentSum = 0;

        for(int i = 0; i < array.length; i++){
            for(int j = i; j < array.length; j++){
                for(int k = i; k <= j; k++){
                    currentSum += array[k];
                }

                if(currentSum > max){
                    max = currentSum;
                }

                currentSum = 0;
            }
        }
        return max;
    }

    /**
     * 令currSum为当前最大子数组的和，maxSum为最后要返回的最大子数组的和，当我们往后扫描时，
     * 对第i+1个元素有两种选择：要么放入前面找到的子数组，要么做为新子数组的第一个元素；
     * 如果currSum加上当前元素a[i]后不小于a[i]，则令currSum加上a[i]，否则currSum重新赋值，置为下一个元素，即currSum = a[i]。
     * 同时，当currSum > maxSum，则更新maxSum = currSum，否则保持原值，不更新。
     * @param array
     * @return
     */
    public int maxChildArraySum2(int[] array){
        int max = array[0];

        int currentSum = 0;

        for(int i = 0; i < array.length; i++){
            currentSum =  array[i] > array[i] + currentSum ? array[i] : currentSum + array[i];
            max = currentSum > max ? currentSum : max;
        }
        return max;
    }
}
